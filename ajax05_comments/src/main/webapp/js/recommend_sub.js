...
function weather(){
    ...
    xhr.onreadystatechange=function(){
        if(xhr.readyState ==4 && xhr.status==200){
            ...
            let data=JSON.parse(xml);
            let item=data.item;
            let degree =""; //기온
            let sky="";//하늘상태
            console.log(item[0].spotName);
            for(var i=0; i<item.length; i++){
                console.log(item[i].spotName);
                if(item[i].spotName == spotName){
                    degree= item[i].th3;
                    sky = item[i].sky;
                }
                degree = "<span>"+degree +"도</span>";
            }
            let status_img = $(".status_img img").src; //날씨 이미지
            let status= $(".weater .status").innerHTML; //온도, 날씨
        }
    }
    let today= new Date();
    let month= today.getMonth()+1; //달
    ...
    let url="http://apis.data.go.kr/1360000/TourStnInfoService/getTourStnVilageFcst?serviceKey="+key+"&numOfRows=10&pageNo=1&CURRENT_DATE="+current_date+"&HOUR=24&COURSE_ID="+course_id;
    console.log(url);
